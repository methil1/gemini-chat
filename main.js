...
// Gemini API の初期化
// 注意: Vite では、環境変数は import.meta.env を介してアクセスされます
const apiKey = import.meta.env.VITE_GEMINI_API_KEY;

if (!apiKey || apiKey === 'YOUR_API_KEY') {
  console.warn("API Key is missing or default. Please check .env file.");
  appendMessage("system", "ERROR: API KEY INVALID. PLEASE CONFIGURE .ENV FILE.");
}
...
function initSession() {
  try {
    if (apiKey && apiKey !== 'YOUR_API_KEY') {
      ...
      // 新しいチャットセッションを作成
      chatSession = ai.chats.create({ model: 'gemini-3-flash-preview' });
